...
void View::DrawInfo()
{
    ...
    auto DisplayTable = [](sortingInfos& sortingInfos, auto& sortedStats, auto&& TranslateEnum)
        {
            ...
            if (sortSpecs && sortSpecs->SpecsCount > 0)
            {
                ...
                {
                    sortingInfos.columnIndex = sortSpec.ColumnIndex;
                    sortingInfos.direction = sortSpec.SortDirection;
                    sortingInfos.dirty = true;
            ...
            std::sort(sortedStats.begin(), sortedStats.end(),
                [&](const auto& a, const auto& b) {
                    switch (sortingInfos.columnIndex)
                    {
                    case 0:
                        return sortingInfos.direction == ImGuiSortDirection_Descending
                            ? TranslateEnum(a.first) > TranslateEnum(b.first)
                            : TranslateEnum(a.first) < TranslateEnum(b.first);
                    case 1:
                        return sortingInfos.direction == ImGuiSortDirection_Descending
            ...
            for (const auto& [type, stats] : sortedStats)
            {
                ...
                ImGui::Text("%s", TranslateEnum(type));
    ...
    if( ImGui::TreeNode( "Queue types statistics" ) )
    {
            ...
            ImGuiWindowFlags_AlwaysVerticalScrollbar ) )
        {
            if( ImGui::BeginTable( "QueueTypeStats", 4, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_Sortable ) )
            {
                ...
                DisplayTable(m_queueTypeSortingInfos, sortedStats, QueueTypeToString);
            }
    ...
    if( ImGui::TreeNode( "Queries statistics" ) )
    {
            ...
            ImGuiWindowFlags_AlwaysVerticalScrollbar ) )
        {
            if( ImGui::BeginTable( "QueryStats", 4, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_Sortable ) )
            {
                ...
                std::vector<std::pair<ServerQuery, Worker::QueueStats>> sortedStats( queryStats.begin(), queryStats.end() );

               
                DisplayTable(m_querySortingInfos, sortedStats, ServerQueryToString);
            }
