...
static constexpr MicroArchUx s_uArchUx[] = {
    { "AMD Zen 4", "Ryzen 5 7600X", "ZEN4" },      { "AMD Zen 3", "Ryzen 5 5600X", "ZEN3" },
    { "AMD Zen 2", "Ryzen 7 3700X", "ZEN2" },      { "AMD Zen+", "Ryzen 5 2600", "ZEN+" },
    { "Alder Lake P", "Core i5-12600K", "ADL-P" }, { "Alder Lake E", "Core i5-12600K", "ADL-E" },
    { "Rocket Lake", "Core i9-11900", "RKL" },     { "Tiger Lake", "Core i7-1165G7", "TGL" },
    { "Ice Lake", "Core i5-1035G1", "ICL" },       { "Cascade Lake", "Core i9-10980XE", "CLX" },
    { "Cannon Lake", "Core i3-8121U", "CNL" },     { "Coffee Lake", "Core i7-8700K", "CFL" },
    { "Kaby Lake", "Core i7-7700", "KBL" },        { "Skylake-X", "Core i9-7900X", "SKX" },
    { "Skylake", "Core i7-6500U", "SKL" },         { "Broadwell", "Core i5-5200U", "BDW" },
    { "Haswell", "Xeon E3-1225 v3", "HSW" },       { "Ivy Bridge", "Core i5-3470", "IVB" },
    { "Sandy Bridge", "Core i7-2600", "SNB" },     { "Westmere", "Core i5-650", "WSM" },
    { "Nehalem", "Core i5-750", "NHM" },           { "Wolfdale", "Core 2 Duo E8400", "WOL" },
    { "Conroe", "Core 2 Duo E6750", "CON" },       { "Bonnell", "Atom D525", "BNL" },
    { "Airmont", "Celeron N3000", "AMT" },         { "Goldmont", "Celeron J3455", "GLM" },
    { "Goldmont Plus", "Celeron J4125", "GLP" },   { "Tremont", "Pentium Silver N6005", "TRM" },
};

static constexpr const char* s_regNameX86[] = {
    "invalid", "rflags", "rax",   "rbx",   "rcx",   "rdx",   "rsi",   "rdi",   "rbp",   "rsp",   "r8",
    "r9",      "r10",    "r11",   "r12",   "r13",   "r14",   "r15",   "mm0",   "mm1",   "mm2",   "mm3",
    "mm4",     "mm5",    "mm6",   "mm7",   "xmm0",  "xmm1",  "xmm2",  "xmm3",  "xmm4",  "xmm5",  "xmm6",
    "xmm7",    "xmm8",   "xmm9",  "xmm10", "xmm11", "xmm12", "xmm13", "xmm14", "xmm15", "xmm16", "xmm17",
    "xmm18",   "xmm19",  "xmm20", "xmm21", "xmm22", "xmm23", "xmm24", "xmm25", "xmm26", "xmm27", "xmm28",
    "xmm29",   "xmm30",  "xmm31", "k0",    "k1",    "k2",    "k3",    "k4",    "k5",    "k6",    "k7" };
static_assert( sizeof( s_regNameX86 ) / sizeof( *s_regNameX86 ) == (size_t)SourceView::RegsX86::NUMBER_OF_ENTRIES,
               "Invalid x86 register name table" );

static SourceView::RegsX86 s_regMapX86[X86_REG_ENDING];

static constexpr const char* s_CostName[] = { "Sample count", "Cycles",       "Branch impact",
                                              "Cache impact", "Retirements",  "Branches taken",
                                              "Branch miss",  "Cache access", "Cache miss" };
...
static size_t CountHwSamples( const SortedVector<Int48, Int48Sort>& vec, const Range& range )
{
    ...
    auto it = std::lower_bound( vec.begin(), vec.end(), range.min,
                                []( const auto& lhs, const auto& rhs ) { return lhs.Val() < rhs; } );
    if( it == vec.end() ) return 0;
    auto end = std::lower_bound( it, vec.end(), range.max,
                                 []( const auto& lhs, const auto& rhs ) { return lhs.Val() < rhs; } );
    return std::distance( it, end );
}

static void PrintHwSampleTooltip( size_t cycles, size_t retired, size_t cacheRef, size_t cacheMiss,
                                  size_t branchRetired, size_t branchMiss, bool hideFirstSeparator )
{
    if( cycles || retired )
    {
...
float SourceView::CalcJumpSeparation( float scale )
{
    ...
    if( m_maxJumpLevel > 75 )
        jsb -= 5;
    else if( m_maxJumpLevel > 60 )
        jsb -= 4;
    else if( m_maxJumpLevel > 45 )
        jsb -= 3;
    else if( m_maxJumpLevel > 30 )
        jsb -= 2;
    else if( m_maxJumpLevel > 15 )
        jsb -= 1;
...
static constexpr uint32_t PackCpuInfo( uint32_t cpuid ) { return ( cpuid & 0xFFF ) | ( ( cpuid & 0xFFF0000 ) >> 4 ); }
...
//                   || ||.- stepping
//                   || |||
static constexpr CpuIdMap s_cpuIdMap[] = { { PackCpuInfo( 0x810F81 ), "ZEN+" },  { PackCpuInfo( 0x800F82 ), "ZEN+" },
                                           { PackCpuInfo( 0x830F10 ), "ZEN2" },  { PackCpuInfo( 0x840F70 ), "ZEN2" },
                                           { PackCpuInfo( 0x860F01 ), "ZEN2" },  { PackCpuInfo( 0x860F81 ), "ZEN2" },
                                           { PackCpuInfo( 0x870F10 ), "ZEN2" },  { PackCpuInfo( 0x890F00 ), "ZEN2" },
                                           { PackCpuInfo( 0x890F80 ), "ZEN2" },  { PackCpuInfo( 0xA00F11 ), "ZEN3" },
                                           { PackCpuInfo( 0xA00F80 ), "ZEN3" },  { PackCpuInfo( 0xA20F10 ), "ZEN3" },
                                           { PackCpuInfo( 0xA20F12 ), "ZEN3" },  { PackCpuInfo( 0xA30F00 ), "ZEN3" },
                                           { PackCpuInfo( 0xA40F00 ), "ZEN3" },  { PackCpuInfo( 0xA40F41 ), "ZEN3" },
                                           { PackCpuInfo( 0xA50F00 ), "ZEN3" },  { PackCpuInfo( 0xA60F12 ), "ZEN4" },
                                           { PackCpuInfo( 0xA10F11 ), "ZEN4" },  { PackCpuInfo( 0x090672 ), "ADL-P" },
                                           { PackCpuInfo( 0x090675 ), "ADL-P" }, { PackCpuInfo( 0x0906A2 ), "ADL-P" },
                                           { PackCpuInfo( 0x0906A3 ), "ADL-P" }, { PackCpuInfo( 0x0906A4 ), "ADL-P" },
                                           { PackCpuInfo( 0x0A0671 ), "RKL" },   { PackCpuInfo( 0x0806C1 ), "TGL" },
                                           { PackCpuInfo( 0x0806D1 ), "TGL" },   { PackCpuInfo( 0x0706E5 ), "ICL" },
                                           { PackCpuInfo( 0x0606A6 ), "ICL" },   { PackCpuInfo( 0x050656 ), "CLX" },
                                           { PackCpuInfo( 0x050657 ), "CLX" },   { PackCpuInfo( 0x060663 ), "CNL" },
                                           { PackCpuInfo( 0x0906EA ), "CFL" },   { PackCpuInfo( 0x0906EB ), "CFL" },
                                           { PackCpuInfo( 0x0906EC ), "CFL" },   { PackCpuInfo( 0x0906ED ), "CFL" },
                                           { PackCpuInfo( 0x0806E9 ), "KBL" },   { PackCpuInfo( 0x0806EA ), "KBL" },
                                           { PackCpuInfo( 0x0906E9 ), "KBL" },   { PackCpuInfo( 0x050654 ), "SKX" },
                                           { PackCpuInfo( 0x0406E3 ), "SKL" },   { PackCpuInfo( 0x0506E0 ), "SKL" },
                                           { PackCpuInfo( 0x0506E3 ), "SKL" },   { PackCpuInfo( 0x0306D4 ), "BDW" },
                                           { PackCpuInfo( 0x040671 ), "BDW" },   { PackCpuInfo( 0x0406F1 ), "BDW" },
                                           { PackCpuInfo( 0x0306C3 ), "HSW" },   { PackCpuInfo( 0x0306F2 ), "HSW" },
                                           { PackCpuInfo( 0x040651 ), "HSW" },   { PackCpuInfo( 0x0306A9 ), "IVB" },
                                           { PackCpuInfo( 0x0306E3 ), "IVB" },   { PackCpuInfo( 0x0306E4 ), "IVB" },
                                           { PackCpuInfo( 0x0206A2 ), "SNB" },   { PackCpuInfo( 0x0206A7 ), "SNB" },
                                           { PackCpuInfo( 0x0206D5 ), "SNB" },   { PackCpuInfo( 0x0206D6 ), "SNB" },
                                           { PackCpuInfo( 0x0206D7 ), "SNB" },   { PackCpuInfo( 0x0206F2 ), "WSM" },
                                           { PackCpuInfo( 0x0206C0 ), "WSM" },   { PackCpuInfo( 0x0206C1 ), "WSM" },
                                           { PackCpuInfo( 0x0206C2 ), "WSM" },   { PackCpuInfo( 0x020652 ), "WSM" },
                                           { PackCpuInfo( 0x020655 ), "WSM" },   { PackCpuInfo( 0x0206E6 ), "NHM" },
                                           { PackCpuInfo( 0x0106A1 ), "NHM" },   { PackCpuInfo( 0x0106A2 ), "NHM" },
                                           { PackCpuInfo( 0x0106A4 ), "NHM" },   { PackCpuInfo( 0x0106A5 ), "NHM" },
                                           { PackCpuInfo( 0x0106E4 ), "NHM" },   { PackCpuInfo( 0x0106E5 ), "NHM" },
                                           { PackCpuInfo( 0x010676 ), "WOL" },   { PackCpuInfo( 0x01067A ), "WOL" },
                                           { PackCpuInfo( 0x0006F2 ), "CON" },   { PackCpuInfo( 0x0006F4 ), "CON" },
                                           { PackCpuInfo( 0x0006F6 ), "CON" },   { PackCpuInfo( 0x0006FB ), "CON" },
                                           { PackCpuInfo( 0x0006FD ), "CON" },   { PackCpuInfo( 0x0106C2 ), "BNL" },
                                           { PackCpuInfo( 0x0106CA ), "BNL" },   { PackCpuInfo( 0x07065A ), "AMT" },
                                           { PackCpuInfo( 0x0506C9 ), "GLM" },   { PackCpuInfo( 0x0506F1 ), "GLM" },
                                           { PackCpuInfo( 0x0706A1 ), "GLP" },   { PackCpuInfo( 0x0706A8 ), "GLP" },
                                           { PackCpuInfo( 0x0806A1 ), "TRM" },   { PackCpuInfo( 0x090661 ), "TRM" },
                                           { PackCpuInfo( 0x0906C0 ), "TRM" },   { 0, 0 } };
...
void SourceView::OpenSymbol( const char* fileName, int line, uint64_t baseAddr, uint64_t symAddr, Worker& worker,
                             const View& view, bool updateHistory )
{
    m_targetLine = line;
...
{
    static constexpr const char* branchX86[] = { "je",   "jne",   "jg",    "jge",  "ja",    "jae",    "jl",     "jle",
                                                 "jb",   "jbe",   "jo",    "jno",  "jz",    "jnz",    "js",     "jns",
                                                 "jcxz", "jecxz", "jrcxz", "loop", "loope", "loopne", "loopnz", "loopz",
                                                 "jnle", "jnl",   "jnge",  "jng",  "jnbe",  "jnb",    "jnae",   "jna",
                                                 "jc",   "jnc",   "jp",    "jpe",  "jnp",   "jpo",    nullptr };
    auto ptr = branchX86;
    while( *ptr )
        if( strcmp( *ptr++, op ) == 0 ) return true;
    return false;
}
...
bool SourceView::Disassemble( uint64_t symAddr, const Worker& worker )
{
    ...
    if( cnt > 0 )
    {
        ...
        for( size_t i = 0; i < cnt; i++ )
        {
            ...
            for( auto j = 0; j < detail.groups_count; j++ )
            {
                if( detail.groups[j] == CS_GRP_JUMP && opType < OpType::Jump )
                    opType = OpType::Jump;
                else if( detail.groups[j] == CS_GRP_BRANCH_RELATIVE && opType < OpType::Branch )
                    opType = OpType::Branch;
                else if( detail.groups[j] == CS_GRP_CALL && opType < OpType::Call )
                    opType = OpType::Call;
                else if( detail.groups[j] == CS_GRP_RET && opType < OpType::Ret )
                    opType = OpType::Ret;
            ...
            if( hasJump )
            {
                ...
                {
                    auto fit = std::lower_bound( insn, insn + cnt, jumpAddr,
                                                 []( const auto& l, const auto& r ) { return l.address < r; } );
            ...
            m_asm.emplace_back( AsmLine{ op.address, jumpAddr, op.mnemonic, op.op_str, (uint8_t)op.size, leaData,
                                         opType, jumpConditional, std::move( params ) } );
        ...
        if( !m_jumpTable.empty() )
        {
            ...
            pdqsort_branchless( jumpRange.begin(), jumpRange.end(),
                                []( const auto& l, const auto& r ) { return l.len < r.len; } );
...
void SourceView::Render( Worker& worker, View& view )
{
    ...
    if( m_symAddr == 0 )
    {
        ...
        else
        {
            ...
            ImGui::SameLine();
            TextColoredUnformatted( ImVec4( 1.f, 0.3f, 0.3f, 1.f ),
                                    "The source file contents might not reflect the actual profiled code!" );
            ImGui::SameLine();
            TextColoredUnformatted( ImVec4( 1.f, 1.f, 0.2f, 1.f ), ICON_FA_TRIANGLE_EXCLAMATION );
        }
...
void SourceView::RenderSymbolView( Worker& worker, View& view )
{
    ...
    ImGui::SameLine();
    ImGui::AlignTextToFramePadding();
    if( ImGui::SmallButton( ICON_FA_ARROW_DOWN_SHORT_WIDE " Entry stacks" ) )
        view.ShowSampleParents( m_symAddr, !m_calcInlineStats );
    if( inlineList )
    {
    ...
    if( inlineList )
    {
        ...
        if( shortenName != ShortenName::Never )
            currSymName = ShortenZoneName( ShortenName::OnlyNormalize, currSymName );
        if( ImGui::BeginCombo( "##functionList", currSymName, ImGuiComboFlags_HeightLarge ) )
        {
            ...
            pdqsort_branchless( symInline.begin(), symInline.end(), []( const auto& l, const auto& r )
            ...
            for( auto& v : symInline )
            {
                ...
                const auto normalized = shortenName != ShortenName::Never
                                            ? ShortenZoneName( ShortenName::OnlyNormalize, symName )
                                            : symName;
                const auto selected =
                    ImGui::Selectable( "", v.first == m_symAddr, ImGuiSelectableFlags_SpanAllColumns );
                ImGui::SameLine( 0, 0 );
    ...
    if( ( as.ipTotalAsm.local + as.ipTotalAsm.ext ) > 0 ||
        ( view.m_statRange.active && worker.GetSamplesForSymbol( m_baseAddr ) ) )
    {
        ImGui::SameLine();
        ImGui::Spacing();
        ImGui::SameLine();
        ...
        if( m_cost == CostType::SampleCount )
        {
            ...
            ImGui::SameLine();
            if( ImGui::SmallButton( m_childCallList ? " " ICON_FA_CARET_UP " " : " " ICON_FA_CARET_DOWN " " ) )
                m_childCallList = !m_childCallList;
            ImGui::SameLine();
            ImGui::Spacing();
            ImGui::SameLine();
            if( m_childCalls )
            {
                TextFocused( ICON_FA_STOPWATCH " Time:",
                             TimeToString( ( as.ipTotalAsm.local + as.ipTotalAsm.ext ) * worker.GetSamplingPeriod() ) );
            }
            else
            {
                TextFocused( ICON_FA_STOPWATCH " Time:",
                             TimeToString( as.ipTotalAsm.local * worker.GetSamplingPeriod() ) );
            }
            if( as.ipTotalAsm.ext )
            {
                ImGui::SameLine();
                ImGui::TextDisabled( "(%c%s)", m_childCalls ? '-' : '+',
                                     TimeToString( as.ipTotalAsm.ext * worker.GetSamplingPeriod() ) );
    ...
    if( m_childCallList )
    {
        ...
        if( !map.empty() )
        {
            ...
            if( ImGui::BeginChild( "ccd", ImVec2( 0, ImGui::GetTextLineHeight() * std::min<size_t>( 4, map.size() ) +
                ...
                for( auto& v : vec )
                {
                    ...
                    *end = '\0';
                    ...
                    if( sd && sd->line != 0 )
                    {
                        ...
                        if( ImGui::IsItemHovered() &&
                            SourceFileValid( fileName, worker.GetCaptureTime(), view, worker ) )
                        {
...
static constexpr uint32_t GoodnessColor[256] = {
    0xFF3232FF, 0xFF3235FE, 0xFF3238FE, 0xFF323AFD, 0xFF323DFD, 0xFF323FFC, 0xFF3242FC, 0xFF3244FB, 0xFF3246FB,
    0xFF3248FB, 0xFF324AFA, 0xFF324CFA, 0xFF324EF9, 0xFF3250F9, 0xFF3252F8, 0xFF3254F8, 0xFF3255F8, 0xFF3257F7,
    0xFF3259F7, 0xFF325AF6, 0xFF325CF6, 0xFF325EF5, 0xFF325FF5, 0xFF3260F4, 0xFF3262F4, 0xFF3263F3, 0xFF3265F3,
    0xFF3266F3, 0xFF3268F2, 0xFF3269F2, 0xFF326AF1, 0xFF326BF1, 0xFF326DF0, 0xFF326EF0, 0xFF326FEF, 0xFF3270EF,
    0xFF3272EE, 0xFF3273EE, 0xFF3274EE, 0xFF3275ED, 0xFF3276ED, 0xFF3277EC, 0xFF3279EC, 0xFF327AEB, 0xFF327BEB,
    0xFF327CEA, 0xFF327DEA, 0xFF327EE9, 0xFF327FE9, 0xFF3280E8, 0xFF3281E8, 0xFF3282E7, 0xFF3283E7, 0xFF3284E6,
    0xFF3285E6, 0xFF3286E5, 0xFF3287E5, 0xFF3288E4, 0xFF3289E4, 0xFF328AE3, 0xFF328BE3, 0xFF328CE2, 0xFF328DE2,
    0xFF328EE1, 0xFF328EE1, 0xFF328FE0, 0xFF3290E0, 0xFF3291DF, 0xFF3292DF, 0xFF3293DE, 0xFF3294DE, 0xFF3295DD,
    0xFF3295DD, 0xFF3296DC, 0xFF3297DC, 0xFF3298DB, 0xFF3299DB, 0xFF329ADA, 0xFF329ADA, 0xFF329BD9, 0xFF329CD9,
    0xFF329DD8, 0xFF329ED8, 0xFF329ED7, 0xFF329FD7, 0xFF32A0D6, 0xFF32A1D5, 0xFF32A2D5, 0xFF32A2D4, 0xFF32A3D4,
    0xFF32A4D3, 0xFF32A5D3, 0xFF32A5D2, 0xFF32A6D2, 0xFF32A7D1, 0xFF32A8D1, 0xFF32A8D0, 0xFF32A9CF, 0xFF32AACF,
    0xFF32AACE, 0xFF32ABCE, 0xFF32ACCD, 0xFF32ADCD, 0xFF32ADCC, 0xFF32AECC, 0xFF32AFCB, 0xFF32AFCA, 0xFF32B0CA,
    0xFF32B1C9, 0xFF32B1C9, 0xFF32B2C8, 0xFF32B3C7, 0xFF32B3C7, 0xFF32B4C6, 0xFF32B5C6, 0xFF32B5C5, 0xFF32B6C5,
    0xFF32B7C4, 0xFF32B7C3, 0xFF32B8C3, 0xFF32B9C2, 0xFF32B9C2, 0xFF32BAC1, 0xFF32BBC0, 0xFF32BBC0, 0xFF32BCBF,
    0xFF32BDBE, 0xFF32BDBE, 0xFF32BEBD, 0xFF32BEBD, 0xFF32BFBC, 0xFF32C0BB, 0xFF32C0BB, 0xFF32C1BA, 0xFF32C2B9,
    0xFF32C2B9, 0xFF32C3B8, 0xFF32C3B7, 0xFF32C4B7, 0xFF32C5B6, 0xFF32C5B5, 0xFF32C6B5, 0xFF32C6B4, 0xFF32C7B3,
    0xFF32C7B3, 0xFF32C8B2, 0xFF32C9B1, 0xFF32C9B1, 0xFF32CAB0, 0xFF32CAAF, 0xFF32CBAF, 0xFF32CCAE, 0xFF32CCAD,
    0xFF32CDAD, 0xFF32CDAC, 0xFF32CEAB, 0xFF32CEAA, 0xFF32CFAA, 0xFF32CFA9, 0xFF32D0A8, 0xFF32D1A8, 0xFF32D1A7,
    0xFF32D2A6, 0xFF32D2A5, 0xFF32D3A5, 0xFF32D3A4, 0xFF32D4A3, 0xFF32D4A2, 0xFF32D5A2, 0xFF32D5A1, 0xFF32D6A0,
    0xFF32D79F, 0xFF32D79E, 0xFF32D89E, 0xFF32D89D, 0xFF32D99C, 0xFF32D99B, 0xFF32DA9A, 0xFF32DA9A, 0xFF32DB99,
    0xFF32DB98, 0xFF32DC97, 0xFF32DC96, 0xFF32DD95, 0xFF32DD95, 0xFF32DE94, 0xFF32DE93, 0xFF32DF92, 0xFF32DF91,
    0xFF32E090, 0xFF32E08F, 0xFF32E18E, 0xFF32E18E, 0xFF32E28D, 0xFF32E28C, 0xFF32E38B, 0xFF32E38A, 0xFF32E489,
    0xFF32E488, 0xFF32E587, 0xFF32E586, 0xFF32E685, 0xFF32E684, 0xFF32E783, 0xFF32E782, 0xFF32E881, 0xFF32E880,
    0xFF32E97F, 0xFF32E97E, 0xFF32EA7D, 0xFF32EA7C, 0xFF32EB7B, 0xFF32EB7A, 0xFF32EC79, 0xFF32EC77, 0xFF32ED76,
    0xFF32ED75, 0xFF32EE74, 0xFF32EE73, 0xFF32EE72, 0xFF32EF70, 0xFF32EF6F, 0xFF32F06E, 0xFF32F06D, 0xFF32F16B,
    0xFF32F16A, 0xFF32F269, 0xFF32F268, 0xFF32F366, 0xFF32F365, 0xFF32F363, 0xFF32F462, 0xFF32F460, 0xFF32F55F,
    0xFF32F55E, 0xFF32F65C, 0xFF32F65A, 0xFF32F759, 0xFF32F757, 0xFF32F855, 0xFF32F854, 0xFF32F852, 0xFF32F950,
    0xFF32F94E, 0xFF32FA4C, 0xFF32FA4A, 0xFF32FB48, 0xFF32FB46, 0xFF32FB44, 0xFF32FC42, 0xFF32FC3F, 0xFF32FD3D,
    0xFF32FD3A, 0xFF32FE38, 0xFF32FE35, 0xFF32FF32,
};
...
void SourceView::RenderSymbolSourceView( const AddrStatData& as, Worker& worker, const View& view, bool hasInlines )
{
    ...
    if( hasInlines && !m_calcInlineStats &&
        ( ( as.ipTotalAsm.local + as.ipTotalAsm.ext ) > 0 ||
          ( view.m_statRange.active && worker.GetSamplesForSymbol( m_baseAddr ) ) ) )
    {
        const auto samplesReady = worker.AreSymbolSamplesReady();
        if( !samplesReady )
    ...
    else
    {
        ...
        if( ImGui::BeginCombo( "##fileList", m_source.filename(), ImGuiComboFlags_HeightLarge ) )
        {
            ...
            else
            {
                ...
                {
                    pdqsort_branchless(
                        fileCountsVec.begin(), fileCountsVec.end(),
                        [&worker]( const auto& l, const auto& r )
                        {
                            return ( l.second.local + l.second.ext == r.second.local + r.second.ext )
                                       ? strcmp( worker.GetString( l.first ), worker.GetString( r.first ) ) < 0
                                       : ( l.second.local + l.second.ext > r.second.local + r.second.ext );
                        } );
                }
                else
                {
                    pdqsort_branchless( fileCountsVec.begin(), fileCountsVec.end(),
                                        [&worker]( const auto& l, const auto& r )
                                        {
                                            return l.second.local == r.second.local
                                                       ? strcmp( worker.GetString( l.first ),
                                                                 worker.GetString( r.first ) ) < 0
                                                       : l.second.local > r.second.local;
                                        } );
                }
                    ...
                    if( hasSamples )
                    {
                        ...
                        if( fit->second.local + fit->second.ext != 0 )
                        {
                            if( m_cost == CostType::SampleCount )
                            {
                                if( m_childCalls )
                                {
                                    ImGui::TextUnformatted( TimeToString( ( fit->second.local + fit->second.ext ) *
                                                                          worker.GetSamplingPeriod() ) );
                                }
                                else
                                {
                                    ImGui::TextUnformatted(
                                        TimeToString( fit->second.local * worker.GetSamplingPeriod() ) );
                                }
                                if( ImGui::IsItemHovered() )
                                {
                                    ImGui::BeginTooltip();
                                    if( fit->second.local )
                                    {
                                        TextFocused( "Local time:",
                                                     TimeToString( fit->second.local * worker.GetSamplingPeriod() ) );
                                        TextFocused( "Local samples:", RealToString( fit->second.local ) );
                                    }
                                    if( fit->second.ext )
                                    {
                                        TextFocused( "Child time:",
                                                     TimeToString( fit->second.ext * worker.GetSamplingPeriod() ) );
                            ...
                            if( m_childCalls )
                            {
                                ImGui::TextDisabled( "(%.2f%%)", 100.f * ( fit->second.local + fit->second.ext ) /
                                                                     ( totalSamples.local + totalSamples.ext ) );
                            }
    ...
    ImGui::SetNextWindowContentSize( ImVec2( m_srcWidth, 0 ) );
    ImGui::BeginChild( "##sourceView", ImVec2( 0, -bottom ), true,
    ...
    if( win->ScrollbarY )
    {
        ...
        if( m_selectedLine != 0 )
        {
            ...
            DrawLine( draw, ImVec2( rect.Min.x + 0.5f, ly + 0.5f ), ImVec2( rect.Max.x + 0.5f, ly + 0.5f ), 0x8899994C,
                      3 );
        }
        if( m_source.idx() == m_hoveredSource && m_hoveredLine != 0 )
        {
            ...
            DrawLine( draw, ImVec2( rect.Min.x + 0.5f, ly + 0.5f ), ImVec2( rect.Max.x + 0.5f, ly + 0.5f ), 0x88888888,
                      3 );
        }
        ...
        pdqsort_branchless( ipData.begin(), ipData.end(),
        ...
        while( it != ipData.end() )
        {
            ...
            {
                const auto color =
                    ( ipSum.local + ipSum.ext == 0 )
                        ? 0x22FFFFFF
                        : GetHotnessColor( ipSum.local + ipSum.ext, as.ipMaxSrc.local + as.ipMaxSrc.ext );
    ...
    if( !m_srcSampleSelect.empty() )
    {
        ...
        if( m_childCalls )
        {
            end = PrintFloat( buf, buf + 16,
                              100.f * ( count.local + count.ext ) / ( as.ipTotalSrc.local + as.ipTotalSrc.ext ), 2 );
        }
...
uint64_t SourceView::RenderSymbolAsmView( const AddrStatData& as, Worker& worker, View& view )
{
    ...
    if( m_cpuArch == CpuArchX64 || m_cpuArch == CpuArchX86 )
    {
        ...
        else
        {
            ...
            if( ImGui::IsItemHovered() )
            {
                ...
                ImGui::BeginTooltip();
                ImGui::TextUnformatted(
                    "Selected microarchitecture does not match the one profiled application was running on" );
                if( m_profileMicroArch >= 0 )
                {
                    ImGui::Text( "Measurements were performed on the %s microarchitecture",
                                 s_uArchUx[m_profileMicroArch].uArch );
    ...
    ImGui::BeginChild( "##asmView", ImVec2( 0, -bottom ), true,
                       ImGuiWindowFlags_NoMove | ImGuiWindowFlags_HorizontalScrollbar );
    SetFont();
        ...
        while( clipper.Step() )
        {
            ...
            if( m_showJumps && !m_jumpTable.empty() && clipper.DisplayStart != clipper.DisplayEnd )
            {
                ...
                for( auto& v : m_jumpTable )
                {
                    ...
                    const auto y0 =
                        ( it0 == insList.end() || *it0 != v.second.min ) ? -th : ( it0 - insList.begin() ) * th;
                    const auto y1 = it1 == insList.end() ? ( insList.size() + 1 ) * th : ( it1 - insList.begin() ) * th;

                    float thickness = m_highlightAddr == v.first ? 2 : 1;
                    if( ImGui::IsWindowHovered() &&
                        ImGui::IsMouseHoveringRect(
                            wpos + ImVec2( xoff + JumpSeparation * ( mjl - v.second.level ) - JumpSeparation / 2,
                                           y0 + th2 ),
                            wpos + ImVec2( xoff + JumpSeparation * ( mjl - v.second.level ) + JumpSeparation / 2,
                                           y1 + th2 ) ) )
                    {
                    ...
                    DrawLine( draw, dpos + ImVec2( xoff + JumpSeparation * ( mjl - v.second.level ), y0 + th2 ),
                              dpos + ImVec2( xoff + JumpSeparation * ( mjl - v.second.level ), y1 + th2 ), col,
                              thickness );
                    ...
                    if( v.first >= minAddr && v.first <= maxAddr )
                    {
                        ...
                        DrawLine( draw, dpos + ImVec2( xoff + JumpSeparation * ( mjl - v.second.level ), y + th2 ),
                                  dpos + ImVec2( xoff + JumpSeparation * mjl + JumpArrow + 1, y + th2 ), col,
                                  thickness );
                        DrawLine( draw, dpos + ImVec2( xoff + JumpSeparation * mjl + JumpArrow, y + th2 ),
                                  dpos + ImVec2( xoff + JumpSeparation * mjl + JumpArrow - th4, y + th2 - th4 ), col,
                                  thickness );
                        DrawLine( draw, dpos + ImVec2( xoff + JumpSeparation * mjl + JumpArrow, y + th2 ),
                                  dpos + ImVec2( xoff + JumpSeparation * mjl + JumpArrow - th4, y + th2 + th4 ), col,
                                  thickness );
                    ...
                    for( auto& s : v.second.source )
                    {
                        if( s >= minAddr && s <= maxAddr )
                        {
                            ...
                            const auto y = ( iit - insList.begin() ) * th;
                            DrawLine( draw, dpos + ImVec2( xoff + JumpSeparation * ( mjl - v.second.level ), y + th2 ),
                                      dpos + ImVec2( xoff + JumpSeparation * mjl + JumpArrow, y + th2 ), col,
                                      thickness );
                        }
                    }
                }
        ...
        if( ImGui::BeginPopup( "jumpPopup" ) )
        {
            ...
            if( s_config.llm )
            {
                ...
                if( ImGui::MenuItem( ICON_FA_ROBOT " Attach jump range in chat" ) )
                {
                    ...
                    nlohmann::json json = {
                        { "type", "assembly" }, { "symbol", symName }, { "code", nlohmann::json::array() } };
                    auto& code = json["code"];
                    ...
                    for( size_t i = start; i < end; i++ )
                    {
                        ...
                        if( srcline != 0 )
                        {
                            line["source"] = { { "file", worker.GetString( srcidx ) }, { "line", srcline } };
                        }
            ...
            if( ImGui::BeginMenu( "Target" ) )
            {
                ...
                else
                {
                    ...
                    auto jumpSym =
                        jumpBase == 0 ? worker.GetSymbolData( m_jumpPopupAddr ) : worker.GetSymbolData( jumpBase );
                    if( jumpSym )
                    {
                        const auto jumpName = worker.GetString( jumpSym->name );
                        const auto normalized = view.GetShortenName() != ShortenName::Never
                                                    ? ShortenZoneName( ShortenName::OnlyNormalize, jumpName )
                                                    : jumpName;
    ...
    if( win->ScrollbarY )
    {
        ...
        if( !m_selectedAddresses.empty() )
        {
            ...
            for( auto& v : lineOff )
            {
                ...
                {
                    lastLine = ly;
                    DrawLine( draw, ImVec2( rect.Min.x + 0.5f, ly + 0.5f ), ImVec2( rect.Max.x + 0.5f, ly + 0.5f ),
                              0x8899994C, 1 );
                }
            }
        }
        ...
        if( m_asmSelected >= 0 )
        {
            ...
            for( int i = 0; i < (int)m_asm.size(); i++ )
            {
                ...
                else if( m_asm[i].regData[0] != 0 )
                {
                    ...
                    uint32_t col = 0;
                    if( ( flags & ( WriteBit | ReadBit ) ) == ( WriteBit | ReadBit ) )
                        col = 0xFF00FFFF;
                    else if( flags & WriteBit )
                        col = 0xFF0000FF;
                    else if( flags & ReadBit )
                        col = 0xFF00FF00;
                    if( col != 0 )
                    {
    ...
    if( !m_asmSampleSelect.empty() )
    {
        ...
        if( m_childCalls )
        {
                              ...
                              100.f * ( count.local + count.ext ) / ( as.ipTotalAsm.local + as.ipTotalAsm.ext ), 2 );
        }
...
void SourceView::RenderLine( const Tokenizer::Line& line, int lineNum, const AddrStat& ipcnt, const AddrStatData& as,
                             Worker* worker, const View* view )
{
    ...
    if( as.ipTotalSrc.local + as.ipTotalSrc.ext != 0 )
    {
        if( ( m_childCalls && ipcnt.local + ipcnt.ext == 0 ) || ( !m_childCalls && ipcnt.local == 0 ) )
        {
            ...
            if( hasHwData && ImGui::IsWindowHovered() &&
                ImGui::IsMouseHoveringRect( wpos, wpos + ImVec2( ts.x * 7, ty ) ) )
            {
                UnsetFont();
                ImGui::BeginTooltip();
        ...
        else
        {
            ...
            {
                hover =
                    PrintPercentage( 100.f * ( ipcnt.local + ipcnt.ext ) / ( as.ipTotalSrc.local + as.ipTotalSrc.ext ),
                                     sit == m_srcSampleSelect.end() ? 0xFFFFFFFF : 0xFF8888FF );
            }
            else
            {
                hover = PrintPercentage( 100.f * ipcnt.local / as.ipTotalSrc.local,
                                         sit == m_srcSampleSelect.end() ? 0xFFFFFFFF : 0xFF8888FF );
            }
            if( hover )
            {
                ...
                if( hasHwData )
                    PrintHwSampleTooltip( cycles, retired, cacheRef, cacheMiss, branchRetired, branchMiss, false );
                ImGui::EndTooltip();
                SetFont();
    ...
    if( showHwSamples )
    {
        ...
        if( hasHwData )
        {
            if( m_hwSamplesRelative )
            {
                ...
                {
                    RenderHwLinePart( cycles, retired, branchRetired, branchMiss, cacheRef, cacheMiss, it->second.local,
                                      as.hwMaxSrc.local, it->second.ext, as.hwMaxSrc.ext, ts );
                }
            }
            else
            {
...
static tracy_force_inline uint32_t AsmColor( uint32_t base, bool inContext, int isSelected )
{
    ...
    else
    {
        ...
        {
        case 0:
            return ( base & 0xFFFFFF ) | 0x88000000;
        case 1:
            return 0x88FFFFFF;
        case 2:
            return 0x882F2FFF;
...
void SourceView::RenderAsmLine( AsmLine& line, const AddrStat& ipcnt, const AddrStatData& as, Worker& worker,
                                uint64_t& jumpOut, int maxAddrLen, int maxAddrLenRel, View& view )
{
    const auto scale = GetScale();
    const auto ty = ImGui::GetTextLineHeight();
    auto draw = ImGui::GetWindowDrawList();
    ...
    if( as.ipTotalAsm.local + as.ipTotalAsm.ext != 0 )
    {
        ...
        else
        {
            ...
            {
                hover =
                    PrintPercentage( 100.f * ( ipcnt.local + ipcnt.ext ) / ( as.ipTotalAsm.local + as.ipTotalAsm.ext ),
                                     sit == m_asmSampleSelect.end() ? 0xFFFFFFFF : 0xFF8888FF );
            }
            else
            {
                hover = PrintPercentage( 100.f * ipcnt.local / as.ipTotalAsm.local,
                                         sit == m_asmSampleSelect.end() ? 0xFFFFFFFF : 0xFF8888FF );
            }
            if( hover )
            {
    ...
    if( showHwSamples )
    {
        ...
        if( hw )
        {
            if( m_hwSamplesRelative )
            {
                ...
                {
                    RenderHwLinePart( cycles, retired, branchRetired, branchMiss, cacheRef, cacheMiss, it->second.local,
                                      as.hwMaxAsm.local, it->second.ext, as.hwMaxAsm.ext, ts );
                }
            }
            else
            {
    ...
    if( m_asmShowSourceLocation && !m_sourceFiles.empty() )
    {
        ...
        if( srcline != 0 )
        {
            ...
            const auto fileColor = GetHsvColor( srcidx.Idx(), 0 );
            ImGui::PushStyleVar( ImGuiStyleVar_FramePadding, ImVec2( 0, 0 ) );
            ImGui::ColorButton( "c1",
                                ImVec4( ( fileColor & 0xFF ) / 255.f, ( ( fileColor >> 8 ) & 0xFF ) / 255.f,
                                        ( ( fileColor >> 16 ) & 0xFF ) / 255.f, 1.f ),
                                ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoDragDrop,
                                ImVec2( ty - 3 * scale, ty - 3 * scale ) );
            ImGui::PopStyleVar();
            ImGui::SameLine();
        ...
        {
            ImGui::PushStyleVar( ImGuiStyleVar_FramePadding, ImVec2( 0, 0 ) );
            ImGui::ColorButton( "c1", ImVec4( 0.f, 0.f, 0.f, 1.f ),
                                ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoDragDrop,
                                ImVec2( ty - 3 * scale, ty - 3 * scale ) );
            ImGui::PopStyleVar();
            ImGui::SameLine();
    ...
    if( m_showJumps )
    {
        ...
        if( jit != m_jumpOut.end() )
        {
            ...
            DrawLine( draw, dpos + ImVec2( xoff + JumpSeparation * mjl + th2, th2 ),
                      dpos + ImVec2( xoff + JumpSeparation * mjl + th2 + JumpArrow / 2, th2 ), col );
            DrawLine( draw, dpos + ImVec2( xoff + JumpSeparation * mjl + th2, th2 ),
                      dpos + ImVec2( xoff + JumpSeparation * mjl + th2 + th4, th2 - th4 ), col );
            DrawLine( draw, dpos + ImVec2( xoff + JumpSeparation * mjl + th2, th2 ),
                      dpos + ImVec2( xoff + JumpSeparation * mjl + th2 + th4, th2 + th4 ), col );
        }
    }
    ...
    if( m_cpuArch == CpuArchX64 || m_cpuArch == CpuArchX86 )
    {
        ...
        {
            static constexpr const char* LeaTable[] = { "LEA",   "LEA_B", "LEA_BD", "LEA_BI", "LEA_BID",
                                                        "LEA_D", "LEA_I", "LEA_ID", "LEA_R",  "LEA_RD" };
        ...
        if( it != m_microArchOpMap.end() )
        {
            ...
            auto oit = std::lower_bound( uarch->ops, uarch->ops + uarch->numOps, opid,
                                         []( const auto& l, const auto& r ) { return l->id < r; } );
            if( oit != uarch->ops + uarch->numOps && ( *oit )->id == opid )
            {
                ...
                if( !res.empty() )
                {
                    pdqsort_branchless( res.begin(), res.end(),
    ...
    if( line.jumpAddr != 0 )
    {
        ...
        if( jumpSym )
        {
            ...
            if( jumpName )
                normalized = view.GetShortenName() != ShortenName::Never
                                 ? ShortenZoneName( ShortenName::OnlyNormalize, jumpName )
                                 : jumpName;
        }
    }
    ...
    ImGui::BeginGroup();
    TextColoredUnformatted( AsmColor( AsmOpTypeColors[(int)line.opType], inContext, isSelected ),
                            line.mnemonic.c_str() );
    ...
    if( line.jumpAddr != 0 )
    {
        ...
        if( lit != m_locMap.end() )
        {
            ImGui::PushStyleColor( ImGuiCol_Text, AsmColor( AsmSyntaxColors[(int)Tokenizer::AsmTokenColor::Label],
    ...
    if( !hasJump && jumpName && jumpBase != m_baseAddr )
    {
        ImGui::PushStyleColor(
            ImGuiCol_Text, AsmColor( AsmSyntaxColors[(int)Tokenizer::AsmTokenColor::Label], inContext, isSelected ) );
        if( jumpOffset == 0 )
        {
            ImGui::Text( "%s", normalized );
        }
    ...
    if( !hasJump )
    {
        ...
        while( ptr < end )
        {
            ...
            TextColoredUnformatted( AsmColor( AsmSyntaxColors[(int)it->color], inContext, isSelected ), it->begin,
                                    it->end );
            ImGui::SameLine( 0, 0 );
            ptr = it->end;
            ++it;
    ...
    if( line.regData[0] != 0 )
    {
        ...
        for( ;; )
        {
            ...
            if( ( line.regData[idx] & ( WriteBit | ReadBit ) ) == ( WriteBit | ReadBit ) )
                col = ImVec4( 1, 1, 0.5f, 1 );
            else if( line.regData[idx] & WriteBit )
                col = ImVec4( 1, 0.5f, 0.5f, 1 );
            else if( line.regData[idx] & ReadBit )
                col = ImVec4( 0.5f, 1, 0.5f, 1 );
            else
            ...
            if( ImGui::IsItemHovered() )
            {
                ImGui::BeginTooltip();
                if( ( line.regData[idx] & ( WriteBit | ReadBit ) ) == ( WriteBit | ReadBit ) )
                    ImGui::TextUnformatted( "Read and write" );
                else if( line.regData[idx] & WriteBit )
                    ImGui::TextUnformatted( "Write" );
                else if( line.regData[idx] & ReadBit )
                    ImGui::TextUnformatted( "Read" );
                else
                    ImGui::TextUnformatted( "Previous read" );
                ImGui::EndTooltip();
            }
            idx++;
        }
...
void SourceView::RenderHwLinePart( size_t cycles, size_t retired, size_t branchRetired, size_t branchMiss,
                                   size_t cacheRef, size_t cacheMiss, size_t branchRel, size_t branchRelMax,
                                   size_t cacheRel, size_t cacheRelMax, const ImVec2& ts )
{
    if( cycles )
    {
...
void SourceView::SelectLine( uint32_t line, const Worker* worker, bool updateAsmLine, uint64_t targetAddr,
                             bool changeAsmLine )
{
    m_selectedLine = line;
...
void SourceView::SelectAsmLines( uint32_t file, uint32_t line, const Worker& worker, bool updateAsmLine,
                                 uint64_t targetAddr, bool changeAsmLine )
{
    m_selectedAddresses.clear();
...
void SourceView::GatherIpHwStats( AddrStatData& as, Worker& worker, const View& view, CostType cost )
{
    ...
    for( auto& v : m_asm )
    {
        ...
        if( view.m_statRange.active )
        {
            switch( cost )
            {
            case CostType::Cycles:
                stat = CountHwSamples( hw->cycles, view.m_statRange );
                break;
            case CostType::Retirements:
                stat = CountHwSamples( hw->retired, view.m_statRange );
                break;
            case CostType::BranchesTaken:
                stat = CountHwSamples( hw->branchRetired, view.m_statRange );
                break;
            case CostType::BranchMiss:
                stat = CountHwSamples( hw->branchMiss, view.m_statRange );
                break;
            case CostType::SlowBranches:
                stat = sqrt( CountHwSamples( hw->branchMiss, view.m_statRange ) *
                             CountHwSamples( hw->branchRetired, view.m_statRange ) );
                break;
            case CostType::CacheAccess:
                stat = CountHwSamples( hw->cacheRef, view.m_statRange );
                break;
            case CostType::CacheMiss:
                stat = CountHwSamples( hw->cacheMiss, view.m_statRange );
                break;
            case CostType::SlowCache:
                stat = sqrt( CountHwSamples( hw->cacheMiss, view.m_statRange ) *
                             CountHwSamples( hw->cacheRef, view.m_statRange ) );
                break;
            default:
                assert( false );
                return;
        ...
        else
        {
            ...
            case CostType::Cycles:
                ...
                break;
            case CostType::Retirements:
                stat = hw->retired.size();
                break;
            case CostType::BranchesTaken:
                stat = hw->branchRetired.size();
                break;
            case CostType::BranchMiss:
                stat = hw->branchMiss.size();
                break;
            case CostType::SlowBranches:
                stat = sqrt( hw->branchMiss.size() * hw->branchRetired.size() );
                break;
            case CostType::CacheAccess:
                stat = hw->cacheRef.size();
                break;
            case CostType::CacheMiss:
                stat = hw->cacheMiss.size();
                break;
            case CostType::SlowCache:
                stat = sqrt( hw->cacheMiss.size() * hw->cacheRef.size() );
                break;
            default:
                assert( false );
                return;
...
void SourceView::CountHwStats( AddrStatData& as, Worker& worker, const View& view )
{
    ...
    for( auto& v : m_asm )
    {
        ...
        if( view.m_statRange.active )
        {
            ...
            {
                branch = sqrt( CountHwSamples( hw->branchMiss, view.m_statRange ) *
                               CountHwSamples( hw->branchRetired, view.m_statRange ) );
            }
            else
            {
            ...
            cache = sqrt( CountHwSamples( hw->cacheMiss, view.m_statRange ) *
                          CountHwSamples( hw->cacheRef, view.m_statRange ) );
        }
        else
        {
...
void SourceView::GatherIpStats( uint64_t baseAddr, AddrStatData& as, const Worker& worker, bool limitView,
                                const View& view )
{
    auto filename = m_source.filename();
    if( limitView )
    {
        ...
        if( !vec ) return;
        auto it = std::lower_bound( vec->begin(), vec->end(), view.m_statRange.min,
                                    []( const auto& lhs, const auto& rhs ) { return lhs.time.Val() < rhs; } );
        if( it == vec->end() ) return;
        auto end = std::lower_bound( it, vec->end(), view.m_statRange.max,
                                     []( const auto& lhs, const auto& rhs ) { return lhs.time.Val() < rhs; } );
...
void SourceView::GatherAdditionalIpStats( uint64_t baseAddr, AddrStatData& as, const Worker& worker, bool limitView,
                                          const View& view )
{
    ...
    if( limitView )
    {
        for( uint64_t ip = baseAddr; ip < baseAddr + sym->size.Val(); ip++ )
        {
            ...
            if( !cp ) continue;
            auto it = std::lower_bound( cp->begin(), cp->end(), view.m_statRange.min,
                                        []( const auto& lhs, const auto& rhs ) { return lhs.time.Val() < rhs; } );
            if( it == cp->end() ) continue;
            auto end = std::lower_bound( it, cp->end(), view.m_statRange.max,
                                         []( const auto& lhs, const auto& rhs ) { return lhs.time.Val() < rhs; } );
...
void SourceView::GatherChildStats( uint64_t baseAddr, unordered_flat_map<uint64_t, uint32_t>& map, Worker& worker,
...
void SourceView::FollowRead( size_t line, RegsX86 reg, size_t limit )
{
    ...
    if( data.jumpAddr != 0 )
    {
        auto fit = std::lower_bound( m_asm.begin(), m_asm.end(), data.jumpAddr,
                                     []( const auto& l, const auto& r ) { return l.addr < r; } );
...
void SourceView::FollowWrite( size_t line, RegsX86 reg, size_t limit )
{
    ...
    if( it != m_jumpTable.end() )
    {
        ...
        {
            auto fit = std::lower_bound( m_asm.begin(), m_asm.end(), v,
                                         []( const auto& l, const auto& r ) { return l.addr < r; } );
...
{
    return !m_calcInlineStats || !worker.HasInlineSymbolAddresses() ||
           worker.GetInlineSymbolForAddress( addr ) == m_symAddr;
}

const std::vector<uint64_t>* SourceView::GetAddressesForLocation( uint32_t fileStringIdx, uint32_t line,
                                                                  const Worker& worker )
{
        ...
        for( auto& op : m_asm )
        {
            ...
            if( frame )
            {
                ...
                for( uint8_t i = 0; i < end; i++ )
                {
                    ...
                    {
                        // In some cases debug data can be nonsense, assigning an inline function to the same line as
                        // the parent. The specific example is in TracySysTrace.cpp:1150, which has the following local
                        // stack trace:
                        //   1. RingBuffer::Enable()        TracySysTrace.cpp:1150
                        //   2. SysTraceWorker()            TracySysTrace.cpp:1150
                        // RingBuffer::Enable() is in TracyRingBuffer.hpp:74, and this line is assigned to the remaining
                        // asm lines generated by this inline function. But for some reason, the parent call site is
                        // assigned for the first asm line, that should be assigned to the inline function itself.
...
void SourceView::Save( const Worker& worker, size_t start, size_t stop )
{
    ...
    Fileselector::SaveFile(
        "asm", "Assembly file",
        [this, &worker, start, stop]( const char* fn )
        {
            FILE* f = nullptr;
