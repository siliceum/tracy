...
#    define TRACY_CL_ASSERT( p )                                                                                       \
        if( !( p ) )                                                                                                   \
        {                                                                                                              \
            TracyMessageL( "TRACY_CL_ASSERT failed on " TracyFile ":" TRACY_CL_TO_STRING( TracyLine ) );               \
            assert( false && "TRACY_CL_ASSERT failed" );                                                               \
        }
#    define TRACY_CL_CHECK_ERROR( err )                                                                                \
        if( err != CL_SUCCESS )                                                                                        \
        {                                                                                                              \
            std::ostringstream oss;                                                                                    \
            oss << "TRACY_CL_CHECK_ERROR failed on " << TracyFile << ":" << TracyLine << ": error code " << err;       \
            auto msg = oss.str();                                                                                      \
...
class OpenCLCtx
{
  public:
        ...
        , m_tail( 0 )
    {
        ...
        TRACY_CL_CHECK_ERROR( clEnqueueWriteBuffer( queue, dummyBuffer, CL_FALSE, 0, sizeof( uint32_t ), &dummyValue, 0,
        ...
        cl_int eventStatus;
        TRACY_CL_CHECK_ERROR( clGetEventInfo( writeBufferEvent, CL_EVENT_COMMAND_EXECUTION_STATUS, sizeof( cl_int ),
                                              &eventStatus, nullptr ) );
        TRACY_CL_ASSERT( eventStatus == CL_COMPLETE );
        TRACY_CL_CHECK_ERROR(
            clGetEventProfilingInfo( writeBufferEvent, CL_PROFILING_COMMAND_END, sizeof( cl_ulong ), &tgpu, nullptr ) );
...
#    endif
        ...
        for( ; m_tail != m_head; m_tail = ( m_tail + 1 ) % QueryCount )
        {
            ...
            cl_int eventStatus;
            cl_int err = clGetEventInfo( eventInfo.event, CL_EVENT_COMMAND_EXECUTION_STATUS, sizeof( cl_int ),
                                         &eventStatus, nullptr );
            if( err != CL_SUCCESS )
            {
                std::ostringstream oss;
                oss << "clGetEventInfo falied with error code " << err << ", on event " << eventInfo.event
                    << ", skipping...";
                auto msg = oss.str();
                TracyMessage( msg.data(), msg.size() );
            ...
            cl_int eventInfoQuery =
                ( eventInfo.phase == EventPhase::Begin ) ? CL_PROFILING_COMMAND_START : CL_PROFILING_COMMAND_END;

            cl_ulong eventTimeStamp = 0;
            err = clGetEventProfilingInfo( eventInfo.event, eventInfoQuery, sizeof( cl_ulong ), &eventTimeStamp,
                                           nullptr );
...
#    endif
    ...
    tracy_force_inline OpenCLCtxScope( OpenCLCtx* ctx, uint32_t line, const char* source, size_t sourceSz,
...
#        define TracyCLNamedZone( ctx, varname, name, active )                                                         \
            ...
            tracy::OpenCLCtxScope varname( ctx, &TracyConcat( __tracy_gpu_source_location, TracyLine ),                \
                                           TRACY_CALLSTACK, active );
#        define TracyCLNamedZoneC( ctx, varname, name, color, active )                                                 \
            static constexpr tracy::SourceLocationData TracyConcat( __tracy_gpu_source_location, TracyLine ){          \
...
#        define TracyCLZoneC( ctx, name, color )                                                                       \
            TracyCLNamedZoneCS( ctx, __tracy_gpu_zone, name, color, TRACY_CALLSTACK, true )
#        define TracyCLZoneTransient( ctx, varname, name, active )                                                     \
            tracy::OpenCLCtxScope varname( ctx, TracyLine, TracyFile, strlen( TracyFile ), TracyFunction,              \
                                           strlen( TracyFunction ), name, strlen( name ), TRACY_CALLSTACK, active );
#    else
#        define TracyCLNamedZone( ctx, varname, name, active )                                                         \
            static constexpr tracy::SourceLocationData TracyConcat( __tracy_gpu_source_location, TracyLine ){          \
                name, TracyFunction, TracyFile, (uint32_t)TracyLine, 0 };                                              \
            tracy::OpenCLCtxScope varname( ctx, &TracyConcat( __tracy_gpu_source_location, TracyLine ), active );
#        define TracyCLNamedZoneC( ctx, varname, name, color, active )                                                 \
            static constexpr tracy::SourceLocationData TracyConcat( __tracy_gpu_source_location, TracyLine ){          \
                name, TracyFunction, TracyFile, (uint32_t)TracyLine, color };                                          \
            tracy::OpenCLCtxScope varname( ctx, &TracyConcat( __tracy_gpu_source_location, TracyLine ), active );
...
#        define TracyCLNamedZoneS( ctx, varname, name, depth, active )                                                 \
            static constexpr tracy::SourceLocationData TracyConcat( __tracy_gpu_source_location, TracyLine ){          \
                name, TracyFunction, TracyFile, (uint32_t)TracyLine, 0 };                                              \
            tracy::OpenCLCtxScope varname( ctx, &TracyConcat( __tracy_gpu_source_location, TracyLine ), depth, active );
#        define TracyCLNamedZoneCS( ctx, varname, name, color, depth, active )                                         \
            static constexpr tracy::SourceLocationData TracyConcat( __tracy_gpu_source_location, TracyLine ){          \
                name, TracyFunction, TracyFile, (uint32_t)TracyLine, color };                                          \
            tracy::OpenCLCtxScope varname( ctx, &TracyConcat( __tracy_gpu_source_location, TracyLine ), depth, active );
#        define TracyCLZoneS( ctx, name, depth ) TracyCLNamedZoneS( ctx, __tracy_gpu_zone, name, depth, true )
#        define TracyCLZoneCS( ctx, name, color, depth )                                                               \
            TracyCLNamedZoneCS( ctx, __tracy_gpu_zone, name, color, depth, true )
#        define TracyCLZoneTransientS( ctx, varname, name, depth, active )                                             \
            tracy::OpenCLCtxScope varname( ctx, TracyLine, TracyFile, strlen( TracyFile ), TracyFunction,              \
...
#        define TracyCLNamedZoneCS( ctx, varname, name, color, depth, active )                                         \
            TracyCLNamedZoneC( ctx, varname, name, color, active )
...
#        define TracyCLZoneTransientS( ctx, varname, name, depth, active )                                             \
            TracyCLZoneTransient( ctx, varname, name, active )
#    endif
