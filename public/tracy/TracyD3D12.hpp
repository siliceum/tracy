...
#    define TracyD3D12Panic( msg, ... )                                                                                \
        do                                                                                                             \
        {                                                                                                              \
            assert( false && "TracyD3D12: " msg );                                                                     \
            TracyMessageLC( "TracyD3D12: " msg, tracy::Color::Red4 );                                                  \
            __VA_ARGS__;                                                                                               \
        } while( false );
  ...
  public:
        ...
        , m_queue( queue )
    {
        ...
        if( queue->GetDesc().Type == D3D12_COMMAND_LIST_TYPE_COPY )
        {
            ...
            HRESULT hr =
                device->CheckFeatureSupport( D3D12_FEATURE_D3D12_OPTIONS3, &featureData, sizeof( featureData ) );
        ...
        heapDesc.Type = queue->GetDesc().Type == D3D12_COMMAND_LIST_TYPE_COPY
        ...
        if( FAILED( device->CreateCommittedResource( &readbackHeapProps, D3D12_HEAP_FLAG_NONE, &readbackBufferDesc,
        ...
        MemWrite( &item->gpuNewContext.thread,
                  decltype( item->gpuNewContext.thread )( 0 ) ); // #TODO: why 0 instead of GetThreadHandle()?
    ...
    ~D3D12QueueCtx()
    {
        ...
        while( m_payloadFence->GetCompletedValue() != m_activePayload ) /* busy-wait ... */;
        Collect();
...
#    endif
  ...
  private:
    ...
    tracy_force_inline uint8_t GetId() const { return m_contextId; }
};
...
#    endif
  ...
  public:
    tracy_force_inline D3D12ZoneScope( D3D12QueueCtx* ctx, ID3D12GraphicsCommandList* cmdList,
                                       const SourceLocationData* srcLocation, bool active )
    ...
    tracy_force_inline D3D12ZoneScope( D3D12QueueCtx* ctx, ID3D12GraphicsCommandList* cmdList,
                                       const SourceLocationData* srcLocation, int32_t depth, bool active )
        : D3D12ZoneScope( ctx, cmdList, active )
    {
        if( !m_active ) return;
    ...
    tracy_force_inline D3D12ZoneScope( D3D12QueueCtx* ctx, uint32_t line, const char* source, size_t sourceSz,
    ...
    tracy_force_inline ~D3D12ZoneScope()
    {
        ...
        m_cmdList->ResolveQueryData( m_ctx->m_queryHeap, D3D12_QUERY_TYPE_TIMESTAMP, m_queryId, 2,
                                     m_ctx->m_readbackBuffer, m_queryId * sizeof( uint64_t ) );
    }
};
...
#    define TracyD3D12SrcLocObject( name, color )                                                                      \
        static constexpr tracy::SourceLocationData TracyD3D12SrcLocSymbol{ name, TracyFunction, TracyFile,             \
                                                                           (uint32_t)TracyLine, color };

#    if defined TRACY_HAS_CALLSTACK && defined TRACY_CALLSTACK
#        define TracyD3D12Zone( ctx, cmdList, name )                                                                   \
            TracyD3D12NamedZoneS( ctx, TracyD3D12UnnamedZone, cmdList, name, TRACY_CALLSTACK, true )
#        define TracyD3D12ZoneC( ctx, cmdList, name, color )                                                           \
            TracyD3D12NamedZoneCS( ctx, TracyD3D12UnnamedZone, cmdList, name, color, TRACY_CALLSTACK, true )
#        define TracyD3D12NamedZone( ctx, varname, cmdList, name, active )                                             \
            TracyD3D12SrcLocObject( name, 0 );                                                                         \
            tracy::D3D12ZoneScope varname{ ctx, cmdList, &TracyD3D12SrcLocSymbol, TRACY_CALLSTACK, active };
#        define TracyD3D12NamedZoneC( ctx, varname, cmdList, name, color, active )                                     \
            TracyD3D12SrcLocObject( name, color );                                                                     \
            tracy::D3D12ZoneScope varname{ ctx, cmdList, &TracyD3D12SrcLocSymbol, TRACY_CALLSTACK, active };
#        define TracyD3D12ZoneTransient( ctx, varname, cmdList, name, active )                                         \
            TracyD3D12ZoneTransientS( ctx, varname, cmdList, name, TRACY_CALLSTACK, active )
#    else
#        define TracyD3D12Zone( ctx, cmdList, name )                                                                   \
            TracyD3D12NamedZone( ctx, TracyD3D12UnnamedZone, cmdList, name, true )
#        define TracyD3D12ZoneC( ctx, cmdList, name, color )                                                           \
            TracyD3D12NamedZoneC( ctx, TracyD3D12UnnamedZone, cmdList, name, color, true )
#        define TracyD3D12NamedZone( ctx, varname, cmdList, name, active )                                             \
            TracyD3D12SrcLocObject( name, 0 );                                                                         \
            tracy::D3D12ZoneScope varname{ ctx, cmdList, &TracyD3D12SrcLocSymbol, active };
#        define TracyD3D12NamedZoneC( ctx, varname, cmdList, name, color, active )                                     \
            TracyD3D12SrcLocObject( name, color );                                                                     \
            tracy::D3D12ZoneScope varname{ ctx, cmdList, &TracyD3D12SrcLocSymbol, active };
#        define TracyD3D12ZoneTransient( ctx, varname, cmdList, name, active )                                         \
            tracy::D3D12ZoneScope varname{ ctx,           TracyLine,                                                   \
...
#        define TracyD3D12ZoneS( ctx, cmdList, name, depth )                                                           \
            TracyD3D12NamedZoneS( ctx, TracyD3D12UnnamedZone, cmdList, name, depth, true )
#        define TracyD3D12ZoneCS( ctx, cmdList, name, color, depth )                                                   \
            TracyD3D12NamedZoneCS( ctx, TracyD3D12UnnamedZone, cmdList, name, color, depth, true )
#        define TracyD3D12NamedZoneS( ctx, varname, cmdList, name, depth, active )                                     \
            TracyD3D12SrcLocObject( name, 0 );                                                                         \
            tracy::D3D12ZoneScope varname{ ctx, cmdList, &TracyD3D12SrcLocSymbol, depth, active };
#        define TracyD3D12NamedZoneCS( ctx, varname, cmdList, name, color, depth, active )                             \
            TracyD3D12SrcLocObject( name, color );                                                                     \
            tracy::D3D12ZoneScope varname{ ctx, cmdList, &TracyD3D12SrcLocSymbol, depth, active };
#        define TracyD3D12ZoneTransientS( ctx, varname, cmdList, name, depth, active )                                 \
...
#        define TracyD3D12NamedZoneS( ctx, varname, cmdList, name, depth, active )                                     \
            TracyD3D12NamedZone( ctx, varname, cmdList, name, active )
#        define TracyD3D12NamedZoneCS( ctx, varname, cmdList, name, color, depth, active )                             \
            TracyD3D12NamedZoneC( ctx, varname, cmdList, name, color, active )
#        define TracyD3D12ZoneTransientS( ctx, varname, cmdList, name, depth, active )                                 \
            TracyD3D12ZoneTransient( ctx, varname, cmdList, name, active )
#    endif
